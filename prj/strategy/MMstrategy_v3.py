...
class MarketBalance(MMTemplate):
    ...
    def onTick(self, tick):
        ...
        # 每隔n个tick进行操作
        self.tickcount += 1
        if not self.OKtickUpdate or not self.tickUpdate or not self.orderUpdate or self.tickcount <= 30:
            return
        else:
            self.tickcount = 0
        ...
        # 2.重新挂单
        for ii in range(len(self.orderbook1)):
            if self.orderbook1[ii][1] > 0:      # 如果是新价位且为委买单，则买入相应数量
                self.buy('BTC_CNY.ZHCOIN', self.orderbook1[ii][0], self.orderbook1[ii][1])
            elif self.orderbook1[ii][1] < 0:    # 如果是新价位且为委卖单，则卖出相应数量
